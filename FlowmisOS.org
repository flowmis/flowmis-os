...
# cp -r ~/FlowmisOS/tangle/.Xressources ~/.Xressources
...
* Linux & Arch
 ...
 1. ~Arch~ mit ~Linux-Kernel~ und der Paketverwaltung ~Pacman~ ist die Grundlage auf dem FlowmisOS aufbaut
...
mkdir -p ~/.config/doom/ &&
cp -r ~/FlowmisOS/tangle/config.el ~/.config/doom/config.el && cp -r ~/FlowmisOS/tangle/init.el ~/.config/doom/init.el && cp -r ~/FlowmisOS/tangle/packages.el ~/.config/doom/packages.el && cp -r ~/FlowmisOS/tangle/banner/ ~/.config/doom/banner/ && cp -r ~/FlowmisOS/tangle/themes/ ~/.config/doom/themes/ && cp -r ~/FlowmisOS/tangle/eshell/ ~/.config/doom/eshell/ && echo '###########################Doom ready###########################' &&
...
#+RESULTS:
| doom-init-fonts-h | doom-init-theme-h | mane-theme-zusatzeinstellungen | org-persist-load-all | doom-init-leader-keys-h | x-wm-set-size-hint | tramp-register-archive-file-name-handler | magit-maybe-define-global-key-bindings | table--make-cell-map |
...
*** Eigenes Theme +  Einstellungen Erscheinungsbild
