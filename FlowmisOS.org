...
1. Arch ISO downloaden & auf Stick oder in VM(other Linux 5.x or later Kernel 64bit und genug Speicher etc. geben)
...
4. folgende Befehle eingeben:
 ...
 #+begin_src
 loadkeys de-latin1
 fdisk /dev/sda             -> n p Enter Enter w
 mkfs.ext4 /dev/sda1
 mount /dev/sda1 /mnt
 ...
 hwclock --systohc
 pacman -S neovim sudo git grub efibootmgr dosfstools os-prober mtools networkmanager
 nvim /etc/locale.conf       -> LANG=de_DE.UTF-8
 nvim /etc/locale.gen        -> Den # vor #de_DE.UTF-8 UTF-8 entfernen
 locale-gen
 nvim /etc/hostname          -> NamePC
 nvim /etc/hosts             -> Einfügen: 127.0.0.1   localhost
                                         ::1         localhost
...
#löschen wenn es hier nicht nötig ist, aber vll erst hier sinnvoll zu installieren?:    pacman -S grub
...
#reboot & login
 ...
 cd ~ FlowmisOS
 . install.sh (oder: . installvm.sh)
 sudo systemctl enable lightdm
 sudo systemctl reboot -> emacs in FlowmisOS.org und org-babel-tangle um configs an ihren Ort zu bringen und erneut reboot
 #+end_src
...
#+begin_src shell :tangle installvm.sh
 mkdir -p ~/.config/{rofi,alacritty,picom,qtile,doom}
 git clone https://github.com/flowmis/DLT.git                   #Klonen der Repos funktioniert nicht wenn sie privat sind und ich kein Token parat hab -> kann man Token mitliefern oder git credentials vorab aus FlowmisOS kopieren???
 git clone https://github.com/flowmis/pres.git
 git clone https://github.com/flowmis/Kivy.git
 ...
 git clone https://aur.archlinux.org/brave-bin.git
 git clone --depth 1 https://github.com/hlissner/doom-emacs ~/.emacs.d
 sudo pacman -S xf86-video-fbdev xorg alacritty base-devel lightdm lightdm-gtk-greeter networkmanager picom pinta htop kdenlive nitrogen qtile pcmanfm rofi deepin-screen-recorder thunderbird emacs flameshot libreoffice gimp vlc
 # xf86-video-fbdev > bei physischen Maschinen Intel, AMD oder Nividia Treiber (siehe ArchWiki)
 # base-devel            -> for makepkg -si
 # lightdm               -> Login into Windowmanager
 # lightdm-gtk-greeter   -> Startbildschirm Loginmanager
