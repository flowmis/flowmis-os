...
4. Installationsskript
   - base-devel ermöglicht den Befehl <makepkg -si> der nötig ist um aus den Brave Binaries ein Paket zu machen das im Anschluss installiert wird
   - lightdm(Login-Manager) + lightdm-gtk-greeter(Start-Bildschirm) sind nötig um ein Login das System zu haben
   - ripgrep braucht man für Doom Emacs (ebenso wie gnu find und fd (sollten jedoch bereits installiert sein - siehe auch github von doom emacs)
