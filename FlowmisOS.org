...
- Dank the-silver-searcher kann ich in einem Ordner alle Dateien auf ihren Inhalt hin untersuchen -> im Terminal in Ordner navigieren und dort ~ag "Suchbegriff"~ eingeben um dann die Dateien mit Kontext angezeigt zu bekommen die den Suchbegriff beinhalten
...
- Einstellen kann ich das Theme durch ~sudo archlinux-tweak-tool~ oder hier: ~sudo nvim /etc/sddm.conf.d/kde_settings.conf~ unter Current -> Current=sugar-dark
