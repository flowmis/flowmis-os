...
Eine EMACS-Funktion erstellen die meine gewohnte Ordnerstruktur herstellt und in die ich mit org-babel-tangle dann im Anschluss alles rein schreibe. Hier einfach evaluieren sollte ausreichen um Funktion auszuführen und Zeug zu erstellen(<Space el> auf Funktionsklammer).
#+begin_src emacs-lisp
(make-directory "~/.config/rofi" t)                         ;error da Teile des gegebenen Pfads noch gar nicht vorhanden sind. Mit t kommt error nicht und sie werden mit erstellt
(make-directory "~/.config/alacritty" t)                       
(make-directory "~/.config/qtile" t)                         
(make-directory "~/.config/picom" t)                         
;(make-directory "~/.config/conky" t)                         
;(make-directory "~/.config/pcmanfm" t)                         
;(make-directory "~/.config/doom" t)                         
;(make-directory "~/.config/dmscripts" t)                         
;(make-directory "~/.config/fish" t)                         
;(make-directory "~/.config/nitogen" t)                         
;(make-directory "~/.config/htop" t)                         
#+end_src

#+begin_src emacs-lisp
(expand-file-name "mane_denkt_sich_was_aus.py" "~/Skripte")     ;erzeugt Path auch wenn noch nicht vorhanden und ist so ganz nützlich für Erstellung meiner gewünschten Funktion -> Rest siehe Lisp und probier bisschen rum
(file-exists-p "~/Skripte/mane_denkt_sich_was_aus.py")          ;nil da ich es ja nicht erstellt hab sondern nur den Pfad dafür gecalled habe -> Auch nützlich ist file-readable-p, file-executable-p, file-writable-p
(make-directory "~/Skripte")                                    ;Beim ersten ausführen erstellt es das dir. Erneut ausführen gibt error da es bereits vorhanden ist. Ergänzt man t kommt kein Error mehr
(delete-directory "~/Skripte")                                  ;ist klar oder
(delete-directory "~/Skripte" t)                                ;löscht das ganze Verzeichnis mit den Unterordnern falls t angegeben ist --> Siehe auch make-file aufschriebe bei proggen_lernen lisp, wobei die Dateit ja auch eh doch org-babel-tangle von allein erstellt wird
(directory-files "~/Skripte" t ".org" nil 3)                    ;gibt maximal 3 .org files in diesem dir aus geordnet(wegen nil). Erstes t sagt dass voller Pfad ausgegeben wird - Parameter den eigenen Wünschen anpassen
(directory-files-recursively "~" "\\.org$")                     ;gibt alle .org files aus die man in irgendwelchen Ordnern und Unterordnern im home Ordner hat
#+end_src
...
- [[#arch][Arch]]
- [[#simple-vm-install][Simple VM Install]]
- [[#installation-guide][Installation Guide]]
- [[#installationsskript-und-programme-für-linux][Installationsskript und Programme für Linux]]
  - [[#setup-linux][Setup Linux]]
- [[#windowmanager-qtile][Windowmanager Qtile]]
  - [[#allgemeines-zu-qtile][Allgemeines zu Qtile]]
  - [[#standard-qtile-config][Standard Qtile Config]]
  - [[#dt-config][DT Config]]
...
- [[#rofi][Rofi]]
- [[#emacs-config-von-0][EMACS Config von 0]]
- [[#doomemacs-config][DoomEmacs config]]
  ...
  - [[#mane-hinzugefügt][Mane hinzugefügt]]
...
* TODO FlowmisOS
+ Eigene FlowmisOS Repo auf Github erstellen mit Installationsskript, Dotfiles, Packages etc.
+ Alacritty grundlegende Config (Aliase, Keybindings, Aussehen)
+ Richtiger Exportpfad für Neuinstallation
+ vimrc und .bashrc alles im Home Verzeichnis speichern und in wiefern nötig für Alacritty oder anderes?
* Export Configs
| Src-Block                      | momentaner Exportpfad (Space mB) | Zielpfad                                     |
|                                | :tangle <NameFile>               | :tangle <Pfad+NameFile>                      |
|--------------------------------+----------------------------------+----------------------------------------------|
| QTILE                          | gleicher Ordner                  | home/flowmis/.config/qtile/config.py         |
| ALACRITTY                      | gleicher Ordner                  | home/flowmis/.config/alacritty/alacritty.yml |
| Picom                          | gleicher Ordner                  | home/flowmis/.config/picom/picom.conf        |
| EMACS                          | not available                    | home/flowmis/.emacs.d/config.el?             |
| Achtung bei DOOM anderer Pfad? |                                  | home/flowmis/.emacs.d/init.el                |
|                                |                                  | home/flowmis/.emacs.d/packages.el?           |
| ROFI                           | not available                    | ?                                            |
| Conky                          | not available                    | ?                                            |
| STARTUP SKRIPT                 | gleicher Ordner                  | home/flowmis/.xprofile                       |
| Login Manager                  | not available                    | ?                                            |
* Arch
Ist ein Desktop Betriebssystem dass auf allen x86_64 kompatiblen Machinen mit circa 1 GiB Ram und etwa 2 GiB Speicher installiert werden kann
* Simple VM Install
+ Arch iso in VM und Einstellungen VM      -> simplest solution ist der Befehl 'archinstall' gleich zu Beginn nach booten auf Iso einzugeben und dem Guide folgen
+ loadkeys de-latin1
+ fdisk //dev/sda/             -> p n Enter Enter w
+ mkfs.ext4 /dev/sda1
+ mount /dev/sda1 /mnt
...
+ locale-gen
+ vim /etc/hostname          -> NamePC
+ vim /etc/hosts             -> Einfügen: 127.0.0.1   localhost
                                          ::1         localhost
...
+ sudo pacman -S
    ...
    xorg                  -> xrandr etc
    nitrogen              -> Wallpaper
    picom                 -> Kompositor for windowmanagment (hilft qtile)
    git
    alacritty
    qtile
...
+ systemctl reboot and install what you want
* Installation Guide
...
3. Installation in der virtuellen Konsole:
    ...
    #+end_src
4. Alternative Arch GUI Installation (nicht FlowmisOS):
   Mit komfortabler GUI und float Window Manager wie man es von Windows kennt:
   - Arch wie oben beschrieben zum laufen bekommen und statt git clone einfach gnome installieren und konfigurieren
    #+begin_src shell
...
#+begin_src shell :tangle installationsskript.sh
sudo pacman -Syu
sudo pacman -S git python python-pip vlc libreoffice flameshot gimp alacritty pinta htop kdenlive virtualbox nitrogen qtile rofi deepin-screen-recorder neovim thunderbird emacs
...
#+begin_src sh :tangle alacritty.yml
...
key_bindings:
    # | heisst 2 Knöpfe gleichzeitig drücken
  - { key: V,         mods: Control|Shift, action: Paste                       }
...
#+end_src
* Git
#+begin_src shell :tangle ~/.git-credentials
https://flowmis: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@github.com
#+end_src  
