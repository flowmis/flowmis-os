...
- [[#send-files-to-places-vorher-space-mb-in-ordner-tangle][Send files to places (vorher: <Space-mB> in Ordner tangle!)]]
- [[#emacs][Emacs]]
  - [[#doomemacs-config][DoomEmacs config]]
  - [[#alte-1-file-configel][Alte 1 file config.el]]
...
* Send files to places (vorher: <Space-mB> in Ordner tangle!)
#+begin_src shell
...
# cp -r ~/FlowmisOS/tangle/eshell/ ~/.config/doom/eshell
echo done
#+end_src
...
** DoomEmacs config
- 3 files um alles zu configurieren sollte reichen config.el init.el und packages.el
- Doom kommt out of the box mit diesen 3 files unter ~/.doom.d/
- Diesen .doom.d Ordner kann ich als Backup speichern, sollte ihn dann aber umbenennen
- ich mache meine config Dateien in ~/.config/doom/ und deshalb sollte vorab ~/.doom.d/ gelöscht oder umbenannt werden um keine Komplikationen zu haben (wahrscheinlich kann ich es aber als fallback auch lassen?)
- In ~/.emacs.d/ finden sich die doom scripte zu doom sync etc. und weiere Dinge wie der cache und eingestellte Dinge - hier kann man bei Problemen mal reinschauen sollte aber nicht zu viel machen!
- Erst lädt meine config.el und dann stellt Doom alles ein! -> Bei der config.el darauf achten dass Doom mein Sachen überschreibt wenn ich kein `after!' block verwende! Viele config blocks aus dem Internet verwenden use-package -> in Doom kann man use-package verwenden, aber wenn man sicher gehen will dass Doom die Einstellungen nicht überschreibt sollte man es in after! ändern
  (after! PACKAGE
    (setq x y))
  - Ausnahmen:
...
*** Org-Mode
**** Grundeinstellungen
- <K> öffnet org-link-abbrev-alist und damit kann ich aus Emacs suchen -> <K> wenn Cursor über einer Emacs Variable/Funktion,... sucht automatisch in den docs und zeigt Beschreibung dazu an
#+begin_src emacs-lisp :tangle tangle/config.el
...
*** Open outside of Emacs
- Datein in einem Programm ausserhalb Emacs aufmachen -> Videos, Word, Excel,... mit <C-c o> aus dired auch mit mehreren markierten Dateien gleichzeitig möglich!
- Habe es deaktiviert weil ich es in dired configuration ebenfalls einstellen kann - mittlerweile überflüssig - als inaktives Ersatzpaket aufbewahren?
#+begin_src emacs-lisp :tangle tangle/config.el
...
#+end_src
*** Ditaa
- statt der Funktion sollte es eigentlich auch mit after! machbar sein momentanes Problem dass der ditaa jar path immer nicht passt bis ich die Funktion ausführe
...
#+end_src
*** Undo
- undo tree kann ich in init.el statt packages eintragen
#+begin_src emacs-lisp :tangle tangle/config.el
...
#+end_src
*** Export (org-export, org-reveal, org-publish)
#+begin_src emacs-lisp :tangle tangle/config.el
;HTML PRÄSENTATIONEN
...
#+end_src
*** Register
- weiß gerade nicht was es ist also vorerst deaktiviert
#+begin_src emacs-lisp :tangle tangle/config.el
(map! :leader
      (:prefix ("r" . "registers")
...
#+end_src
*** Sonstiges noch irgendwo unterbringen falls ich es brauche
- checken ob ich es brauche
#+begin_src emacs-lisp :tangle tangle/config.el
;; (use-package emojify
