...
|-------------------------------------+---------------------------------------------------------------------------------------------------------------|
| nix flake update                    | Update der Pakete auf neueste Version                                                                         |
| sudo nixos-rebuild switch --flake . | System updaten auf die bereits mit dem Befehl zuvor aktualisierte flake.log                                   |
3. Download und Install [[https://nixos.org/][hier]]
  - Am besten in VM aufsetzen, dann die Konfigurationsdatei speichern und mit dieser dann ein Gerät auf das System ändern!
  - VM sollte haben: 8GB RAM, 4 Kerne und min. 100G an Speicher um alles richtig kompilieren, updaten und aufsetzen zu können!
  - Wenn ich qtile nutzen will macht es eigentlich bei der Installation Sinn bei Desktop der verwendet werden soll keinen auszuwählen. Dann komme ich in die tty und kann dort qtile einrichten (mit fertiger Konfiguration brauche ich eigentlich auch keine grafische Installation und kann die minimale .iso herunterladen und meine Konfiguration dort einspielen)
4. Einrichten
  - mkdir .dotfiles unter /home/flowmis erstellen und dort rein gehen um dann die Konfigurationsdateien dort hinzukopieren: ~cp /etc/nixos/configuration.nix .~ ~cp /etc/nixos/hardware-configuration.nix .~ (hier easy mit git zu managen) 
  - in diesem Ordner nun mit vim eine weitere Datei mit folgendem Inhalt erstellen: ~vim flake.nix~
#+begin_src 
{
