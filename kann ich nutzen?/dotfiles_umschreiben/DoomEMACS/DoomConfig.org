#+TITLE: Flowmis Emacs Config
#+AUTHOR: Flowmis
#+DESCRIPTION: Credits got to DT for the base config
#+STARTUP: showeverything
Zusätzlich zu dieser config wichtig sind init.el, packages.el und config.el --> ändere ich etwas in der init.el dann muss ich in der Konsole ".emacs.d/bin/doom sync" eingeben um Änderungen zu aktivieren. Aus diesem org Dokument kann man die config.el generieren lassen in dem man den Code von hier heraus exportiert in die Datei config.el (Literate Programming)
