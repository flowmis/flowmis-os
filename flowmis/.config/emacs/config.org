...
- [[#org][Org]]
  ...
  - [[#roam][Roam]]
  - [[#schreib-präsentationsmodus][Schreib-/Präsentationsmodus]]
  - [[#settings][Settings]]
...
(defun mane-start-mode-enable ()
  ...
  ;; Anpassungen für Schriftarten und Größen
  (setq-local face-remapping-alist                     ;;vielleicht sollte ich auch hier mit fontaine arbeiten wie bei meinem Schreib/-Präsentationsmodus?
              '((default (:height 1.5))
                (header-line (:height 4.5))
                (org-document-title (:height 2.0) :weight bold)
                (org-block-begin-line (:height 0.7))))
  ;; Schriftart und Größe für mane-start-mode anpassen
  (buffer-face-mode 1))

(defun mane-start-mode-disable ()
  "Funktionen, die beim Deaktivieren von mane-start-mode ausgeführt werden."
  (olivetti-mode -1)
  ;; Schriftart-Anpassungen rückgängig machen
...
(with-eval-after-load 'org
  ...
  (define-key org-mode-map (kbd "M-s") #'mane-writing-and-presentation-mode)
  (define-key org-mode-map (kbd "M-S") #'logos-narrow-dwim)
...
;;         org-roam-ui-open-on-start t))
#+end_src
** Schreib-/Präsentationsmodus
- Schriftart und Spacing mit Fontaine und Spacious-Padding sind im Unterpunkt Erscheinungsbild festgelegt. Ich habe in fontaine die Variable large auf die ich mich hier beziehen kann, um die Schriftart in einem Modus anzupassen und kann theoretisch auch anderen dort hinzufügen!?
- Keybind zum togglen unter org-mode keybindings eingerichtet
#+begin_src elisp
;; Schreibmodus Funktion
...
(defun mane-writing-and-presentation-mode ()
  "Aktivieren oder Deaktivieren des Schreib- und Präsentationsmodus."
  (interactive)
  (if mane-writing-mode-active
      ...
      (progn
        ...
        (message "Schreib- und Präsentationsmodus deaktiviert."))
    ;; Modus aktivieren
    (progn
      (hide-mode-line-mode 1)
      (feebleline-mode -1)
      (spacious-padding-mode 1)
      (fontaine-set-preset 'large) ;; Zum Schreib-/Präsentations-Setup wechseln
      ...
      (message "Schreib- und Präsentationsmodus aktiviert."))))
#+end_src
** Settings
