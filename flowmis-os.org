...
# cp -u ~/flowmis-os/tangle/.Xresources ~/.Xresources
...
- Mein id_rsa1 und id_rsa2 (privater ssh keys) und id_rsa1.pub id_rsa1.pub in den Ordner ~/.ssh einfügen (Inhalt aus Passwortmanager)
- chmod der id_rsa Dateien auf 600 bevor man sie hinzufügt ~chmod 600 ~~/.ssh/id_rsa~
- folgende Datei erstellen, dass die keys hinzugefügt werden beim start - danach neusstarten und checken mit ~ssh-add -l~ anzeigen lassen ob die keys erflogreich hinzugefügt wurden (.gitconfig brauche ich dann eigentlich nicht mehr wirklich?)
  #+begin_src sh :tangle ~/.config/autostart/ssh-agent-startup.sh
#!/bin/bash
eval "$(ssh-agent -s)"
ssh-add /home/flowmis/.ssh/id_rsa1
ssh-add /home/flowmis/.ssh/id_rsa2
  #+end_src
  #+begin_src sh :tangle ~/.ssh/config
Host *
  AddKeysToAgent yes
  UseKeychain yes
...
sudo pacman -S viewnior deepin-screen-recorder flameshot gimp pinta vlc kdenlive thunderbird libreoffice virtualbox gpa rofi ditaa
#+end_src
...
#+begin_src sh :tangle tangle/.Xresources
