...
- id_rsa (privater ssh key) in ~/.ssh kopieren bzw. erstellen und aus passwortmanager den inhalt hineinkopieren und dann die wichtigen Rechte geben mit ~chmod 600 ~~/.ssh/id_rsa~
- id_rsa.pub ebenfalls in ~/.ssh einfügen und schauen dass er mit dem auf github oder wo auch immer übereinstimmt - nun sollte ssh-agent eine id ausgeben und damit zeigen dass es funktioniert -> nun sollte ich eigentlich nichts weiter brauchen um mit dem git repo zu kommunizieren!
Neuen Key erstellen:
